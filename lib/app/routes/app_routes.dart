part of 'app_pages.dart';
// DO NOT EDIT. This is code generated via package:get_cli/get_cli.dart

abstract class Routes {
  Routes._();

  static const HOME = _Paths.HOME;
  static const AUTHENTICATION = _Paths.AUTHENTICATION;
  static const CLIENTS_PAGE = _Paths.CLIENTS_PAGE;
  static const NOTIFICATIONS = _Paths.NOTIFICATIONS;
  static const USERS = _Paths.USERS;
  static const ITEMS = _Paths.ITEMS;
  static const ITEM_GROUPS = _Paths.ITEM_GROUPS;
  static const INVENTORY_ADJUSTMENTS = _Paths.INVENTORY_ADJUSTMENTS;
  static const CUSTOMERS = _Paths.CUSTOMERS;
  static const SALES_ORDERS = _Paths.SALES_ORDERS;
  static const VENDORS = _Paths.VENDORS;
  static const PURCHASE_ORDERS = _Paths.PURCHASE_ORDERS;
}

abstract class _Paths {
  _Paths._();

  static const HOME = '/home';
  static const AUTHENTICATION = '/authentication';
  static const CLIENTS_PAGE = '/clients-page';
  static const NOTIFICATIONS = '/notifications';
  static const USERS = '/users';
  static const ITEMS = '/items';
  static const ITEM_GROUPS = '/item-groups';
  static const INVENTORY_ADJUSTMENTS = '/inventory-adjustments';
  static const CUSTOMERS = '/customers';
  static const SALES_ORDERS = '/sales-orders';
  static const VENDORS = '/vendors';
  static const PURCHASE_ORDERS = '/purchase-orders';
}

///costume rootRoute always check for latest updates...
const rootRoute = '/';

const homePageDisplayName = "Home";
const homePageRoute = "/home";

const inventoryDropDownDisplayName = "Inventory";
const inventoryDropDownPageRoute = "/inventoryDropDown";

const itemsPageDisplayName = "Items";
const itemsPageRoute = "/items";

const itemGroupsPageDisplayName = "ItemGroups";
const itemGroupsPageRoute = "/item-groups";

const inventoryAdjustmentsPageDisplayName = "Inventory Adjustments";
const inventoryAdjustmentsPageRoute = "/inventory-adjustments";

const salesPageDisplayName = "Sales";
const salesPageRoute = "/sales";

const customersPageDisplayName = "Customers";
const customersPageRoute = "/customers";

const salesOrdersPageDisplayName = "Sales Orders";
const salesOrdersPageRoute = "/sales-orders";

const purchasesPageDisplayName = "Purchases";
const purchasesPageRoute = "/purchases";

const vendorsPageDisplayName = "Vendors";
const vendorsPageRoute = "/vendors";

const purchaseOrdersPageDisplayName = "Purchase Orders";
const purchaseOrdersPageRoute = "/purchase-orders";

const clientsPageDisplayName = "Clients";
const clientsPageRoute = "/clients-page";

const authenticationPageDisplayName = "Log out";
const authenticationPageRoute = "/auth";
